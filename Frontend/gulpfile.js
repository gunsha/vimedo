...
gulp.task('component',function(){
  ...
  gulp.src('./dev/js/templates/componentes/*.**')
    .pipe(template({
      name: name,
...
gulp.task('seccion',function(){
  ...
  gulp.src('./dev/js/templates/seccion/tempCtrl.js')
    .pipe(template({
      name: name,
      camelCaseName: camel(name)
    }))
    ...
    gulp.src('./dev/js/templates/seccion/tempService.js')
    .pipe(template({
      name: name,
      camelCaseName: camel(name)
    }))
...
gulp.task('css',function(){
  ...
  gulp.src(['./dev/css/app.css','./dev/js/lib/angular-growl-v2/build/angular-growl.min.css'])
  .pipe(concat('app.css'))
  .pipe(cssMin())
  .pipe(gulp.dest('./css'));
});
...
gulp.task('libs',function(){
  ...
  for (var i = 0; i < libs.length; i++) {
    console.log('./dev/js/lib/'+libs[i]+'.js');
   ...
   request.addListener('end', function () {
    ...
    if(request.url.indexOf('api')>-1 || request.url.indexOf('auth')>-1){
      ...
      if(request.url.indexOf('.')>-1){
        ...
        file.serveFile('starter.html', 200, {}, request, response);
      }
    }
  }).resume();
 }).listen(3001);
